//
// Created by Julian Guarin on 30/10/23.
//
...
std::vector<std::vector<unsigned char>> opus::Encoder::Encode(
    ...
    unsigned long long totalSize = (unsigned long long) num_channels_ * sample_size;
    const auto frame_length = (unsigned long long) frame_size * totalSize;
    auto data_length = pcm.size() * sample_size;
    if (data_length % frame_length != 0u) {
        //LOG(WARNING) << "PCM samples contain an incomplete frame. Ignoring the incomplete frame.";
...
std::vector<unsigned char> opus::Encoder::EncodeFrame(
    ...
    int frame_size) {
    auto totalSize = (unsigned long long)(frame_size * num_channels_);
    const auto frame_length = (totalSize * sizeof(*frame_start));
    std::vector<unsigned char> encoded(frame_length);
    int32_t encoded_size = static_cast<opus_int32>(encoded.size());
    ...
    encoded.resize((size_t) num_bytes);
...
opus::Decoder::Decoder(opus_uint32 sample_rate, int num_channels)
    : num_channels_(num_channels) {
    int error{};
    decoder_.reset(opus_decoder_create(static_cast<opus_int32>(sample_rate), num_channels, &error));
    valid_ = error == OPUS_OK;
}
...
std::vector<opus_int16> opus::Decoder::Decode(
    ...
    const unsigned long long totalSize = (unsigned long long) (num_channels_ *frame_size);
    const auto frame_length = (totalSize * sizeof(opus_int16));
    std::vector<opus_int16> decoded(frame_length);
    auto num_samples = opus_decode(decoder_.get(), packet.data(), static_cast<opus_int32>(packet.size()),
        decoded.data(), frame_size, decode_fec);
    if (num_samples < 0) {
        //LOG(ERROR) << "Decode error: " << opus::ErrorToString(num_samples);
        return {};
    }
    decoded.resize((size_t)(num_samples * num_channels_));
    return decoded;
}
