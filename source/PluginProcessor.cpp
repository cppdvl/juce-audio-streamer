                                              ...
                                              juce::MidiBuffer& midiMessages)
{
    ...
    for (auto blockIndex = 0lu; blockIndex < splittedBlocks.size(); blockIndex++)
    {
        auto& audioMixerBlock = mAudioMixerBlocks[blockIndex];
        audioMixerBlock.mix(timeStamp, splittedBlocks[blockIndex], blocksToStream, 0); //The ID 0 indicates is the playhead (AudioBuffer) the source of data.
        mixedBlocks.push_back(audioMixerBlock.getBlock(timeStamp));
    }

    jassert(blocksToStream.size() <2); //PEER TO PEER
    ...
    if(streamIdOutput)
    {
        ...
        auto spStreamOut = ptruvgrtp->GetStream(streamIdOutput);
        for (auto& streamID_blocks : blocksToStream)
        {
            /*********************************************/
            /*********************************************/
            auto mixedBlocksToStream = streamID_blocks.second;
            std::vector<std::vector<float>> iMixedBlocks{};
            Utilities::Data::interleaveBlocks(iMixedBlocks, mixedBlocksToStream);
            //PEER TO PEER only the first from iMixedBlocks.
            auto iMixedBlock = iMixedBlocks[0];

            //ENCODE
            //Push the interleaved blocks into the OPUS ENCODER.
            auto [result, encodedBlocks, encodedDataSizeInBytes] = pOpusCodec->encodeChannel(iMixedBlock.data(), 0);
